...
//	} true && false
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String encryptedPassword = DataValidation.encryptPassword(customerpassword); //because db has encrypted pwd
		
		CustomerDAO dao = BSFactory.getCustomerDAO();
		
		CustomerService customerService = BSFactory.getCustomerService(dao);
		
		Customer cus = customerService.loginCustomer(customerEmail, encryptedPassword);
